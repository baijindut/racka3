...
Chordino::FeatureSet
Chordino::process(const float *const *inputBuffers, RealTime timestamp)
{   
    ...
    **/
	...
	if (m_useHMM == 1.0) {
        ...
        */
        ...
        for (vector<RealTime>::iterator it = timestamps.begin(); it != timestamps.end(); ++it) { // initialise the score chordogram
            vector<int> temp = vector<int>(nChord,0);
            scoreChordogram.push_back(temp);
        }
        for (vector<RealTime>::iterator it = timestamps.begin(); it < timestamps.end()-2*halfwindowlength-1; ++it) {
        ...
        count = 0;
        for (vector<RealTime>::iterator it = timestamps.begin(); it != timestamps.end(); ++it) {
