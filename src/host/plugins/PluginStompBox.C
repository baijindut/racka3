...
#include <stdlib.h>
#include <math.h>
#include "PluginStompBox.h"



PluginStompBox::PluginStompBox ()
{
    ...
    _intBuffer = new StereoBuffer(PERIOD);

    cleanup ();

    registerPlugin(1,"StompBox","Distortion",1);
    registerParam(1,"High","","","","",-64,32,1,0);
    registerParam(2,"Mid","","","","",-64,32,1,0);
    registerParam(3,"Low","","","","",-64,32,1,0);
    registerParam(4,"Gain","","","","",0,127,1,63);
    const char* labels[] = {"Odie",
							"Grunge",
							"Rat",
							"Fat Cat",
							"Dist+",
 ...
 */
int
PluginStompBox::process (StereoBuffer* input)
{
    int i;

    // copy input so we dont dirty it. that isnt allowed.
    for (i=0;i<PERIOD;i++)
    {
    	_intBuffer->left[i]=input->left[i];
    	_intBuffer->right[i]=input->right[i];
    }

	float* smpsl = _intBuffer->left;
	float* smpsr = _intBuffer->right;
 ...
 * Parameter control
 */
void PluginStompBox::init_mode (int value)
{
    int tinput = 1;
    float finput = 80.0f;
    float qinput = 1.0f;
...
/*
void
PluginStompBox::setPreset (int npreset)
{
...
void
PluginStompBox::setParam (int npar, int value)
{
    switch (npar) {
//    case 0:
//        setvolume (value);
...
int
PluginStompBox::getParam (int npar)
{
    switch (npar) {
//    case 0:
//        return (Pvolume);
